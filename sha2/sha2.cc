...
/* 右移并保留低8位 */
static uint8_t _shb(uint32_t x, uint32_t n) {
  return ((x >> (n & 31)) & 0xff);
}

/* 左移移并保留32位 */
static uint32_t _shw(uint32_t x, uint32_t n) {
  return ((x << (n & 31)) & 0xffffffff);
}

/* right rotation */
static uint32_t _r(uint32_t x, uint8_t n) {
  return ((x >> n) | _shw(x, 32 - n));
}

/* Ch 操作 */
static uint32_t _Ch(uint32_t x, uint32_t y, uint32_t z) {
  return ((x & y) ^ ((~x) & z));
}

/* Maj 操作 */
static uint32_t _Ma(uint32_t x, uint32_t y, uint32_t z) {
  return ((x & y) ^ (x & z) ^ (y & z));
}

/* S0 */
static uint32_t _S0(uint32_t x) {
  return (_r(x, 2) ^ _r(x, 13) ^ _r(x, 22));
}

/* S1 */
static uint32_t _S1(uint32_t x) {
  return (_r(x, 6) ^ _r(x, 11) ^ _r(x, 25));
}

/* To cacuate Wt */
static uint32_t _G0(uint32_t x) {
  return (_r(x, 7) ^ _r(x, 18) ^ (x >> 3));
}

/* To cacuate Wt */
static uint32_t _G1(uint32_t x) {
...
/* 将数据加入到缓冲区 */
void sha2_rocc_t::_addbits(sha256_context *ctx, uint32_t n)
{
...
/* sha256运算 */
void sha2_rocc_t::_hashop(sha256_context *ctx)
{
...
/* sha256常量初始化 */
void sha2_rocc_t::sha256Init(sha256_context *ctx)
{
  if (ctx != NULL) {
...
void sha2_rocc_t::sha256Hash(sha256_context *ctx, const void *data, size_t len)
{
...
void sha2_rocc_t::sha256Done(sha256_context *ctx, uint8_t *hash)
{
  uint32_t i, j;
...
void sha2_rocc_t::sha256ONE(const void *data, size_t len, uint8_t *hash)
{
  sha256_context ctx;
  sha256Init(&ctx);
  sha256Hash(&ctx, data, len);
  sha256Done(&ctx, hash);
}


define_custom_func(sha2_rocc_t, "sha2", sha2_custom1, custom1)

std::vector<insn_desc_t> sha2_rocc_t::get_instructions()
{
  std::vector<insn_desc_t> insns;
  push_custom_insn(insns, ROCC_OPCODE1, ROCC_OPCODE_MASK, ILLEGAL_INSN_FUNC, sha2_custom1);
  return insns;
}

std::vector<disasm_insn_t*> sha2_rocc_t::get_disasms()
{
